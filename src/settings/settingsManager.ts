...
export class SettingsManager {
    ...
    private getHtml(): string {
        ...
        <style>
            @font-face {
                ...
                font-style: normal;
                font-display: swap;
                unicode-range: U+0000-00FF, U+0131, U+0152-0153, U+02BB-02BC, U+02C6, U+02DA, U+02DC, U+2000-206F, U+2074, U+20AC, U+2122, U+2191, U+2193, U+2212, U+2215, U+FEFF, U+FFFD, U+0400-04FF;
            }
            * {
                margin: 0;
                padding: 0;
                box-sizing: border-box;
                font-family: 'SC-Font', 'Segoe UI', 'Roboto', 'Arial', sans-serif;
            ...
            .example-json {
                font-family: 'Consolas', 'Monaco', 'Courier New', 'Segoe UI', monospace;
                font-size: 11px;
                color: var(--text-primary);
                margin: 0;
        ...
        <div class="settings-panel">
            ...
            <div class="setting-group">
                <h2 data-i18n="geoUnblock">Geo-Unblock</h2>
                <div class="setting-item">
                    <span data-i18n="enableGeoUnblock">Enable Geo-Unblock</span>
                    <label class="toggle">
                        <input type="checkbox" id="geoUnblock" ${this.store.get('geoUnblock') ? 'checked' : ''}>
                        <span class="slider"></span>
                    </label>
                </div>
                <div class="description" data-i18n="geoUnblockDescription">Bypass geographical restrictions on tracks. Allows you to listen to region-blocked content.</div>
            </div>
            ...
            <div class="setting-group">
                ...
                <div class="input-group" id="proxyFields" style="display: ${this.store.get('proxyEnabled') ? 'block' : 'none'
                    ...
                    <div class="setting-item" style="margin-bottom: 10px;">
                        <span data-i18n="proxyPreset">${this.translationService.translate('proxyPreset')}</span>
                        <select class="theme-selector" id="proxyPreset">
                            <option value="custom" data-i18n="proxyCustom">${this.translationService.translate('proxyCustom')}</option>
                            <option value="proxy1">🇺🇸 US - 38.154.227.167:5868</option>
                            <option value="proxy2">🇩🇪 DE - 185.162.229.164:80</option>
                            <option value="proxy3">🇳🇱 NL - 45.140.143.77:8080</option>
                            <option value="proxy4">🇬🇧 UK - 51.158.68.68:8811</option>
                            <option value="proxy5">🇫🇷 FR - 51.158.172.165:8811</option>
                            <option value="proxy6">🇵🇱 PL - 185.238.228.180:80</option>
                        </select>
                    </div>
                ...
                <div class="description" data-i18n="proxyDescription">${this.translationService.translate('proxyDescription')}</div>
            </div>
            ...
            <div class="setting-group">
                ...
                <div class="input-group" id="lastFmFields" style="display: ${this.store.get('lastFmEnabled') ? 'block' : 'none'
                    ...
                    <input type="text" class="textInput" id="lastFmApiKey" placeholder="${this.translationService.translate('lastFmApiKey')}" data-i18n-placeholder="lastFmApiKey" value="${this.store.get('lastFmApiKey') || ''
            }">
                    <input type="password" class="textInput" id="lastFmSecret" placeholder="${this.translationService.translate('lastFmApiSecret')}" data-i18n-placeholder="lastFmApiSecret" value="${this.store.get('lastFmSecret') || ''
            }">
                </div>
  ...
  "originUrl": "https://soundcloud.com/track-url"
            ...
            <div class="setting-group">
                ...
                <div class="setting-item">
                    <span data-i18n="showLyrics">${this.translationService.translate('showLyrics')}</span>
                    <label class="toggle">
                        <input type="checkbox" id="displayLyrics" ${this.store.get('displayLyrics', true) ? 'checked' : ''}>
                        <span class="slider"></span>
                    </label>
                </div>
                <div class="setting-item" id="lyricsModeContainer" style="display: ${this.store.get('displayLyrics', true) ? 'flex' : 'none'}">
                    <span data-i18n="lyricsMode">${this.translationService.translate('lyricsMode')}</span>
                    <select id="rpcLyricsMode" class="theme-selector" style="min-width: 120px;">
                        <option value="line" ${this.store.get('rpcLyricsMode') === 'line' ? 'selected' : ''} data-i18n="lyricsModeLine">${this.translationService.translate('lyricsModeLine')}</option>
                        <option value="multiline" ${this.store.get('rpcLyricsMode') === 'multiline' ? 'selected' : ''} data-i18n="lyricsModeMultiline">${this.translationService.translate('lyricsModeMultiline')}</option>
                        <option value="karaoke" ${this.store.get('rpcLyricsMode') === 'karaoke' ? 'selected' : ''} data-i18n="lyricsModeKaraoke">${this.translationService.translate('lyricsModeKaraoke')}</option>
                        <option value="karaoke_custom" ${this.store.get('rpcLyricsMode') === 'karaoke_custom' ? 'selected' : ''} data-i18n="lyricsModeKaraokeCustom">${this.translationService.translate('lyricsModeKaraokeCustom')}</option>
                    </select>
                </div>
                <div class="input-group" id="karaokeCustomTextContainer" style="display: ${this.store.get('rpcLyricsMode') === 'karaoke_custom' ? 'block' : 'none'}">
                    <input type="text" class="textInput" id="karaokeCustomText" placeholder="${this.translationService.translate('karaokeCustomTextPlaceholder')}" data-i18n-placeholder="karaokeCustomTextPlaceholder" value="${this.store.get('karaokeCustomText') || ''}">
                </div>
                <div class="setting-item" id="karaokeRevealModeContainer" style="display: ${(this.store.get('rpcLyricsMode') === 'karaoke' || this.store.get('rpcLyricsMode') === 'karaoke_custom') ? 'flex' : 'none'}">
                    <span data-i18n="karaokeRevealMode">${this.translationService.translate('karaokeRevealMode')}</span>
                    <select id="karaokeRevealMode" class="theme-selector" style="min-width: 120px;">
                        <option value="word" ${this.store.get('karaokeRevealMode', 'word') === 'word' ? 'selected' : ''} data-i18n="karaokeRevealWord">${this.translationService.translate('karaokeRevealWord')}</option>
                        <option value="char" ${this.store.get('karaokeRevealMode') === 'char' ? 'selected' : ''} data-i18n="karaokeRevealChar">${this.translationService.translate('karaokeRevealChar')}</option>
                    </select>
                </div>
                <div class="setting-item" id="karaokeCustomTransitionContainer" style="display: ${this.store.get('rpcLyricsMode') === 'karaoke_custom' ? 'flex' : 'none'}">
                    <span data-i18n="karaokeCustomTransition">${this.translationService.translate('karaokeCustomTransition')}</span>
                    <select id="karaokeCustomTransition" class="theme-selector" style="min-width: 120px;">
                        <option value="instant" ${this.store.get('karaokeCustomTransition', 'instant') === 'instant' ? 'selected' : ''} data-i18n="karaokeTransitionInstant">${this.translationService.translate('karaokeTransitionInstant')}</option>
                        <option value="char" ${this.store.get('karaokeCustomTransition') === 'char' ? 'selected' : ''} data-i18n="karaokeTransitionChar">${this.translationService.translate('karaokeTransitionChar')}</option>
                    </select>
                </div>
                <div class="setting-item" id="autoDetectSpeedContainer" style="display: ${this.store.get('displayLyrics', true) ? 'flex' : 'none'}"
                    <span data-i18n="autoDetectLyricsSpeed">${this.translationService.translate('autoDetectLyricsSpeed')}</span>
                    <label class="toggle">
                        <input type="checkbox" id="autoDetectLyricsSpeed" ${this.store.get('autoDetectLyricsSpeed', true) ? 'checked' : ''}>
                        <span class="slider"></span>
                    </label>
                </div>
                <div class="setting-item" id="lyricsSpeedContainer" style="display: ${this.store.get('displayLyrics', true) && !this.store.get('autoDetectLyricsSpeed', true) ? 'flex' : 'none'}">
                    <span data-i18n="lyricsSpeed">${this.translationService.translate('lyricsSpeed')}</span>
                    <div class="input-with-unit">
                        <input type="number" class="textInput" id="lyricsSpeedMultiplier" min="0.5" max="2.0" step="0.05" value="${this.store.get('lyricsSpeedMultiplier', 1.0)}">
                        <span class="unit-symbol">x</span>
                    </div>
                </div>
                <div class="description" id="lyricsSpeedDesc" style="display: ${this.store.get('displayLyrics', true) ? 'block' : 'none'}" data-i18n="lyricsSpeedDescription">${this.translationService.translate('lyricsSpeedDescription')}</div>
                <div class="setting-item" id="usePlainLyricsContainer" style="display: ${this.store.get('displayLyrics', true) ? 'flex' : 'none'}">
                    <span data-i18n="usePlainLyrics">${this.translationService.translate('usePlainLyrics')}</span>
                    <label class="toggle">
                        <input type="checkbox" id="usePlainLyrics" ${this.store.get('usePlainLyrics', false) ? 'checked' : ''}>
                        <span class="slider"></span>
                    </label>
                </div>
                <div class="description" id="usePlainLyricsDesc" style="display: ${this.store.get('displayLyrics', true) ? 'block' : 'none'}" data-i18n="usePlainLyricsDescription">${this.translationService.translate('usePlainLyricsDescription')}</div>
                <div class="setting-item">
                    <span data-i18n="useArtistInStatusLine">${this.translationService.translate('useArtistInStatusLine')}</span>
                    <label class="toggle">
                        <input type="checkbox" id="useArtistInStatusLineToggle" ${(this.store.get('statusDisplayType') as number) === 1 ? 'checked' : ''
            }>
                        <span class="slider"></span>
                    </label>
                </div>
                <div class="setting-item">
                    <span data-i18n="displayMode">${this.translationService.translate('displayMode')}</span>
                    <select id="displayMode" class="theme-selector" style="min-width: 150px;">
                        <option value="rpc" ${this.store.get('displayMode', 'rpc') === 'rpc' ? 'selected' : ''} data-i18n="displayModeRpc">${this.translationService.translate('displayModeRpc')}</option>
                        <option value="custom" ${this.store.get('displayMode') === 'custom' ? 'selected' : ''} data-i18n="displayModeCustom">${this.translationService.translate('displayModeCustom')}</option>
                        <option value="combined" ${this.store.get('displayMode') === 'combined' ? 'selected' : ''}>Combined</option>
                    </select>
                </div>
                <div class="input-group" id="customStatusFields" style="display: ${this.store.get('displayMode') === 'custom' || this.store.get('displayMode') === 'combined' ? 'block' : 'none'}">
                    <input type="password" class="textInput" id="discordToken" placeholder="${this.translationService.translate('discordTokenPlaceholder')}" data-i18n-placeholder="discordTokenPlaceholder" value="${this.store.get('discordToken', '')}">
                </div>
                <div class="description" id="customStatusDesc" style="display: ${this.store.get('displayMode') === 'custom' || this.store.get('displayMode') === 'combined' ? 'block' : 'none'}" data-i18n="customStatusDescription">${this.translationService.translate('customStatusDescription')}</div>
                <div class="setting-item">
            ...
            <div class="setting-group" id="combinedDisplaySettings" style="display: ${this.store.get('displayMode') === 'combined' ? 'block' : 'none'}">
                <h2>
                ...
                <div class="setting-item">
                    ...
                    <select id="combinedDisplayMode" class="theme-selector" style="min-width: 120px;">
                        <option value="rpc" ${this.store.get('combinedDisplayMode', 'both') === 'rpc' ? 'selected' : ''}>RPC Only</option>
                        <option value="status" ${this.store.get('combinedDisplayMode', 'both') === 'status' ? 'selected' : ''}>Status Only</option>
                        <option value="both" ${this.store.get('combinedDisplayMode', 'both') === 'both' ? 'selected' : ''}>Both</option>
                    </select>
                </div>
                <div class="setting-item">
                    <span>Количество строк для RPC</span>
                    <div class="input-with-unit">
                        <input type="number" class="textInput" id="rpcLinesCount" min="1" max="5" value="${this.store.get('rpcLinesCount', 2)}">
                        <span class="unit-symbol">строк</span>
                    </div>
                </div>
                <div class="setting-item">
                    <span>Количество строк для статуса</span>
                    <div class="input-with-unit">
                        <input type="number" class="textInput" id="statusLinesCount" min="1" max="3" value="${this.store.get('statusLinesCount', 1)}">
                        <span class="unit-symbol">строк</span>
                    </div>
                </div>
                <div class="description">
                    Настройки комбинированного отображения текста лирики в RPC и статусе Discord. 
                    В режиме "Both" первые строки идут в RPC, следующие - в Discord статус.
                </div>
            </div>
    ...
    private getJavaScript(): string {
        return `
            ...
            const proxyPresets = {
                'proxy1': { host: '38.154.227.167', port: '5868' },
                'proxy2': { host: '185.162.229.164', port: '80' },
                'proxy3': { host: '45.140.143.77', port: '8080' },
                'proxy4': { host: '51.158.68.68', port: '8811' },
                'proxy5': { host: '51.158.172.165', port: '8811' },
                'proxy6': { host: '185.238.228.180', port: '80' }
            };

            document.getElementById('proxyPreset').addEventListener('change', (e) => {
                ...
                if (preset !== 'custom' && proxyPresets[preset]) {
                    ...
                    ipcRenderer.send('setting-changed', { key: 'proxyHost', value: proxyPresets[preset].host });
                    ipcRenderer.send('setting-changed', { key: 'proxyPort', value: proxyPresets[preset].port });
                }
            });

            // Handle proxy host and port changes
            document.getElementById('proxyHost').addEventListener('change', (e) => {
                document.getElementById('proxyPreset').value = 'custom';
            ...
            document.getElementById('geoUnblock').addEventListener('change', (e) => {
                ipcRenderer.send('setting-changed', { key: 'geoUnblock', value: e.target.checked });
            });
            ...
            document.getElementById('displayLyrics').addEventListener('change', (e) => {
                const isEnabled = e.target.checked;
                ipcRenderer.send('setting-changed', { key: 'displayLyrics', value: isEnabled });
                const container = document.getElementById('lyricsModeContainer');
                const autoDetectContainer = document.getElementById('autoDetectSpeedContainer');
                const speedContainer = document.getElementById('lyricsSpeedContainer');
                const speedDesc = document.getElementById('lyricsSpeedDesc');
                const plainLyricsContainer = document.getElementById('usePlainLyricsContainer');
                const plainLyricsDesc = document.getElementById('usePlainLyricsDesc');
                const autoDetectEnabled = document.getElementById('autoDetectLyricsSpeed')?.checked ?? true;
                if (container) container.style.display = isEnabled ? 'flex' : 'none';
                if (autoDetectContainer) autoDetectContainer.style.display = isEnabled ? 'flex' : 'none';
                if (speedContainer) speedContainer.style.display = isEnabled && !autoDetectEnabled ? 'flex' : 'none';
                if (speedDesc) speedDesc.style.display = isEnabled ? 'block' : 'none';
                if (plainLyricsContainer) plainLyricsContainer.style.display = isEnabled ? 'flex' : 'none';
                if (plainLyricsDesc) plainLyricsDesc.style.display = isEnabled ? 'block' : 'none';
            });

            document.getElementById('rpcLyricsMode').addEventListener('change', (e) => {
                ipcRenderer.send('setting-changed', { key: 'rpcLyricsMode', value: e.target.value });
                const customTextContainer = document.getElementById('karaokeCustomTextContainer');
                const revealModeContainer = document.getElementById('karaokeRevealModeContainer');
                const transitionContainer = document.getElementById('karaokeCustomTransitionContainer');
                const isKaraoke = e.target.value === 'karaoke' || e.target.value === 'karaoke_custom';
                const isKaraokeCustom = e.target.value === 'karaoke_custom';
                if (customTextContainer) customTextContainer.style.display = isKaraokeCustom ? 'block' : 'none';
                if (revealModeContainer) revealModeContainer.style.display = isKaraoke ? 'flex' : 'none';
                if (transitionContainer) transitionContainer.style.display = isKaraokeCustom ? 'flex' : 'none';
            });

            document.getElementById('karaokeCustomText')?.addEventListener('change', (e) => {
                ipcRenderer.send('setting-changed', { key: 'karaokeCustomText', value: e.target.value });
            });

            document.getElementById('karaokeRevealMode')?.addEventListener('change', (e) => {
                ipcRenderer.send('setting-changed', { key: 'karaokeRevealMode', value: e.target.value });
            });

            document.getElementById('karaokeCustomTransition')?.addEventListener('change', (e) => {
                ipcRenderer.send('setting-changed', { key: 'karaokeCustomTransition', value: e.target.value });
            });

            document.getElementById('autoDetectLyricsSpeed').addEventListener('change', (e) => {
                const isEnabled = e.target.checked;
                ipcRenderer.send('setting-changed', { key: 'autoDetectLyricsSpeed', value: isEnabled });
                const speedContainer = document.getElementById('lyricsSpeedContainer');
                if (speedContainer) speedContainer.style.display = !isEnabled ? 'flex' : 'none';
            });

            document.getElementById('lyricsSpeedMultiplier').addEventListener('change', (e) => {
                const value = Math.max(0.5, Math.min(2.0, parseFloat(e.target.value) || 1.0));
                e.target.value = value;
                ipcRenderer.send('setting-changed', { key: 'lyricsSpeedMultiplier', value: value });
            });
            ...
            document.getElementById('displayMode')?.addEventListener('change', (e) => {
                ...
                document.getElementById('customStatusFields').style.display = (mode === 'custom' || mode === 'combined') ? 'block' : 'none';
                document.getElementById('customStatusDesc').style.display = (mode === 'custom' || mode === 'combined') ? 'block' : 'none';
                document.getElementById('combinedDisplaySettings').style.display = mode === 'combined' ? 'block' : 'none';
                ipcRenderer.send('setting-changed', { key: 'displayMode', value: mode });
            });

            document.getElementById('discordToken')?.addEventListener('change', (e) => {
                ipcRenderer.send('setting-changed', { key: 'discordToken', value: e.target.value });
            });
            ...
            // Combined Display Settings обработчики
            document.getElementById('combinedDisplayMode')?.addEventListener('change', (e) => {
                const mode = e.target.value;
                ipcRenderer.send('setting-changed', { key: 'combinedDisplayMode', value: mode });
            });

            document.getElementById('rpcLinesCount')?.addEventListener('change', (e) => {
                const value = Math.max(1, Math.min(5, parseInt(e.target.value) || 2));
                e.target.value = value;
                ipcRenderer.send('setting-changed', { key: 'rpcLinesCount', value: value });
            });

            document.getElementById('statusLinesCount')?.addEventListener('change', (e) => {
                const value = Math.max(1, Math.min(3, parseInt(e.target.value) || 1));
                e.target.value = value;
                ipcRenderer.send('setting-changed', { key: 'statusLinesCount', value: value });
            });
